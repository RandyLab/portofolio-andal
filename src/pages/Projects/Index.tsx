...
export default function Projects() {
							...
							<div key={i} className="space-y-2">
								...
								<div className="flex flex-wrap gap-2 mt-2">
									{[1, 2, 3, 4].map(j => (
										<div
											key={j}
											className="h-6 w-16 bg-gray-100 rounded"
										></div>
									))}
								</div>
							</div>
	...
	const languages: LanguageStat[] = Object.entries(languageStats).map(
		([name, count]) => ({
			name,
			count
		})
		...
		<section className="min-h-screen mt-6 py-12 px-8 text-gray-900">
			<div className="max-w-5xl mx-auto">
				...
				<div className="mb-16">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						{data.repos?.slice(0, 4).map((repo: Repo) => (
							<a
								key={repo.id}
								href={repo.html_url}
								target="_blank"
